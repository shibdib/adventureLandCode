...
function findLocalMonsters(type, returnArray = false) {
    ...
    monsters = Object.values(parent.entities).filter(mob => mob.mtype === type && getMonsterDPS(mob) < partyHealPower());
...
function findBestMonster(minXp, array = false) {
    ...
    for (let x = 0; x < 150; x++) {
        ...
        sorted = sortEntitiesByXp(monsterTypes.filter((v, i, a) => a.indexOf(v) === i)).filter((m) => getMonsterDPS(m) < partyHPS
