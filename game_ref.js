...
var real_id = "", character = null, map = null, resources_loaded = false, socket_ready = false, socket_welcomed = false,
    game_loaded = false, friends = [];
var ch_disp_x = 0, ch_disp_y = 0;
var head_x = 0, head_y = 0;
var tints = [];
var entities = {}, future_entities = {players: {}, monsters: {}}, pull_all = false, pull_all_next = false,
    pulling_all = true, prepull_target_id = null;
...
var map_tiles = [], map_entities = [], map_machines = {}, dtile_size = 32, dtile_width = 0, dtile_height = 0;
...
var current_map = "main", draw_map = "main";
...
var code_run = false, code_active = false, actual_code = false, CC = {};
...
setInterval(function () {
    ...
    var b = mssince(heartbeat), a = new Date();
    if (b > 900) {
        slow_heartbeats++
    } else {
        if (b < 600) {
...
function reset_topleft() {
    ...
    if (ctarget && ctarget.type == "monster" && last_target_cid != ctarget.cid) {
        var c = ctarget;
        var f = G.monsters[c.mtype], e = "", a = f.name;
        if (c.dead) {
            a += " X", c.hp = 0
        }
        if (c.level > 1) {
        ...
        }, {name: "XP", color: "green", value: c.xp},];
        if (c.attack) {
            g.push({name: "ATT", color: "#316EE6", value: c.attack, stunned: c.s.stunned, poisoned: c.s.poisoned})
        }
        if (f.evasion) {
            g.push({name: "EVASION", color: "gray", value: f.evasion + "%"})
        }
        if (f.reflection) {
            g.push({name: "REFLECT.", color: "gray", value: f.reflection + "%"})
        }
        if (f.dreturn) {
            g.push({name: "D.RETURN", color: "gray", value: f.dreturn + "%"})
        }
        if (f.armor) {
            g.push({name: "ARMOR", color: "gray", value: f.armor})
        }
        if (f.resistance) {
            g.push({name: "RESIST.", color: "gray", value: f.resistance})
        }
        if (f.rpiercing) {
            g.push({name: "PIERCE.", color: "gray", value: f.rpiercing})
        }
        if (f.apiercing) {
            g.push({name: "PIERCE.", color: "gray", value: f.apiercing})
        }
        if (f.immune) {
            g.push({line: "IMMUNE", color: "#AEAEAE"})
        }
        if (f.cooperative) {
            g.push({line: "COOPERATIVE", color: "#AEAEAE"})
        }
        if (f.spawns) {
            g.push({line: "SPAWNS", color: "#AEAEAE"})
        }
        if (f.ability) {
            g.push({name: "ABILITY", color: "#FC5F39", value: f.ability.toUpperCase()})
        }
        if (c.target) {
            g.push({name: "TRG", color: "orange", value: c.target})
        }
        if (c.pet) {
            g = [{name: "NAME", value: c.name, color: "#5CBD97"}, {name: "PAL", value: c.owner, color: "#CF539B"},]
        }
        if (c.heal) {
            g.push({line: "SELF HEALING", color: "#9E6367"})
        }
        if (f.explanation) {
            g.push({line: f.explanation, color: "gray"});
            e = "max-width: 200px"
        }
        ...
        if (ctarget && ctarget.npc) {
            var g = [{name: "NPC", color: "gray", value: ctarget.name}, {
                name: "LEVEL",
                color: "orange",
                value: ctarget.level
            },];
            ...
            if (ctarget && ctarget.type == "character" && last_target_cid != ctarget.cid) {
                ...
                if (b.code) {
                    g.push({name: "CODE", color: "gold", value: "Active"})
                }
                if (b.party) {
                    g.push({name: "PARTY", color: "#FF4C73", value: b.party})
                } else {
        ...
        (b.events || []).forEach(function (g) {
            if (g.type == "mhit") {
                ...
                if (g.type == "heal") {
                    ...
                    d_text("+" + g.heal, e, {color: colors.heal});
...
function demo_entity_logic(b) {
    ...
    var a = [[1, 0], [0, 1], [-1, 0], [0, -1], [0.8, 0.8], [-0.8, -0.8], [0.8, -0.8], [-0.8, 0.8]], c = 12;
    if (Math.random() < 0.3) {
        c *= 2
    } else {
        if (Math.random() < 0.3) {
...
function init_socket() {
    ...
    if (location.protocol == "https:") {
        window.socket = io("wss://" + server_addr + ":" + server_port, {secure: true, transports: ["websocket"]})
    } else {
        window.socket = io(server_addr + ":" + server_port, {transports: ["websocket"]})
    }
                            ...
                            if (data.type == "gold_sent") {
                                ...
                                if (data.type == "item_sent") {
                                    ...
                                    if (data.type == "mlevel") {
                                        ...
                                        if (m) {
                                            d_text("+1", m, {color: "#9C76D3", size: "huge"})
                                        }
                                    } else {
                                        if (data.type == "throw") {
        ...
        draw_trigger(function () {
            ...
            if (entity && owner && data.damage !== undefined) {
                ...
                if (data.reflect) {
                    ...
                    if (owner && owner.skin == "konami") {
                        d_line(owner, entity, {color: random_one(["#63388F", "#D1B416", "#CF3327", "#2D82D2"])})
                    } else {
                        if (data.anim == "taunt") {
                            d_line(owner, entity, {color: "taunt"})
                        } else {
                            if (data.anim == "poisonarrow") {
                                d_line(owner, entity, {color: colors.poison, size: 2}), color = colors.poison
                            } else {
                                if (data.anim == "burst") {
            ...
            if (entity && data.heal !== undefined) {
                ...
                d_text("+" + data.heal, entity, {color: colors.heal})
            }
        })
    });
...
function npc_right_click(c) {
    ...
    if (this.role == "xmas_tree") {
        socket.emit("interaction", {type: "xmas_tree"})
    }
    ...
    if (a.interaction) {
        ...
        render_interaction({auto: true, skin: this.skin, message: b})
    }
    if (this.stype == "full") {
...
function map_click(e) {
    ...
    if (e && e.data && e.data.global) {
        var a = e.data.global.x, g = e.data.global.y;
        d = a - width / 2;
        c = g - height / 2;
    ...
    if (q.stype == "full") {
        ...
        if ((q.moving || a) && q.walking === null) {
            if (q.last_stop && msince(q.last_stop) < 320) {
                ...
                reset_ms_check(q, "walk", 350), q.walking = 1
            }
        } else {
    ...
    if (q.mtype && !(no_graphics || paused)) {
        if (q.mtype == "dice") {
            if (q.shuffling) {
                ...
                if (!q.locked) {
                    if (q.line) {
                        remove_sprite(q.line), delete q.line
                    }
...
function add_monster(d) {
    ...
    if (0 && G.dimensions[d.type]) {
        var e = G.dimensions[d.type], a = b.anchor;
...
function player_effects_logic(a) {
    ...
    if (a.s.darkblessing && !a.fx.db) {
        a.fx.db = true;
        start_emblem(a, "dp1", {frames: 1200, no_dip: false})
    } else {
        if (!a.s.darkblessing && a.fx.db) {
            delete a.fx.db;
            stop_emblem(a, "dp1")
    ...
    if (a.s.warcry && !a.fx.wcry) {
        a.fx.wcry = true;
        start_emblem(a, "r1", {frames: 1200, no_dip: false})
    } else {
        if (!a.s.warcry && a.fx.wcry) {
            delete a.fx.wcry;
            stop_emblem(a, "r1")
...
function create_map() {
    ...
    if (cached_map) {
        ...
        for (var e = 0; e < 3; e++) {
            ...
            for (var D = 0; D < GEO.placements.length; D++) {
                ...
                if (N[3] === undefined) {
                    var s = GEO.tiles[N[0]], k = s[3], E = s[4];
    ...
    if (GEO.groups) {
        for (var u = 0; u < GEO.groups.length; u++) {
            ...
            q.type = "group";
            var o = 999999999, r = 99999999, F = -999999999;
            for (var D = 0; D < GEO.groups[u].length; D++) {
                var N = GEO.groups[u][D], s = GEO.tiles[N[0]];
                if (N[1] < r) {
                    r = N[1]
                }
                if (N[2] < o) {
...
function retile_the_map() {
    ...
    if (GEO["default"] && !mdraw_tiling_sprites) {
        for (var d = k; d <= B + 10; d += GEO["default"][3]) {
            for (var c = g; c <= A + 10; c += GEO["default"][4]) {
                var z = floor(d / GEO["default"][3]), v = floor(c / GEO["default"][4]), u = "d" + z + "-" + v;
                if (p[u]) {
                    continue
                }
    ...
    for (var q = 0; q < GEO.placements.length; q++) {
        ...
        if (D[3] === undefined) {
            ...
            var j = GEO.tiles[D[0]], e = j[3], t = j[4];
