...
function findLocalTargets(type, returnArray = false) {
    ...
    potentialTargets = Object.values(parent.entities).filter(mob => mob.mtype === type && getMonsterDPS(mob, true) < partyHPS() && !targetFriends(mob));
    if (isPvP()) {
...
function getGankTargets() {
    ...
    if (players.length && character.map !== 'bank') {
        players = players.filter((p) => (p.level < character.level || p.hp < character.hp) && !p.rip && !parent.friends.includes(p.owner));
